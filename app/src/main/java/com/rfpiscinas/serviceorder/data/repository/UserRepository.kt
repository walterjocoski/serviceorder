...
class UserRepository @Inject constructor(
    ...
    fun getAll(): Flow<List<User>> = userDao.getAll().map { list ->
        list.map { it.toModel() }
    }

    fun getEmployees(): Flow<List<User>> = userDao.getEmployees().map { list ->
        list.map { it.toModel() }
    }

    fun getActiveEmployees(): Flow<List<User>> = userDao.getActiveEmployees().map { list ->
        list.map { it.toModel() }
    }
